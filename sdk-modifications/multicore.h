/*
* MODIFY SDK FUNCTION WITH THIS CONTENT!!!!!!!!!
 */
...
/*! \brief  Run code on core 1 - custom
 *  \ingroup pico_multicore_custom
 *
 * This function essentially is a combination of multicore_lauch_core1 and its call to launch multicore_lauch_core1_with_stack, where the final call to multicore_lauch_core1_raw is modified with a different vtor table pointer
 * 
 * Wake up (a previously reset) core 1 and enter the given function on core 1 using the default core 1 stack (below core 0 stack).
 *
 * core 1 must previously have been reset either as a result of a system reset or by calling \ref multicore_reset_core1
 *
 * core 1 will use the specified vector table, different from core 0
 *
 * \param entry Function entry point
 * \param vector_table Address of the vector table to use for core 1
 * \see multicore_launch_core1_raw
 */
void multicore_lauch_core1_separate_vtor_table(void (*entry)(void), uint32_t vector_table);
